...
Singleton {
    ...
    function autoSelectInterface() {
        ...
        // Hardcoded LAN adapter
        const lanAdapter = "enp4s0";
        for (const [type, ifaces] of interfaces) {
            if (ifaces.includes(lanAdapter)) {
                activeInterface = lanAdapter;
                networkType = type;
                BarState.data.network.activeInterface = lanAdapter;
                BarState.data.network.type = type;
                return;
            }
        }

        // Try preferred interface from config
        const preferred = Config.data.network?.preferredInterface;
        if (preferred && typeof preferred === 'string' && preferred.length > 0) {
            for (const [type, ifaces] of interfaces) {
                if (ifaces.includes(preferred)) {
                    activeInterface = preferred;
                    networkType = type;
                    BarState.data.network.activeInterface = preferred;
                    BarState.data.network.type = type;
                    return;
                }
            }
